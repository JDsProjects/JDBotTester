...
class Bot(commands.Cog):
    ...
    async def status_task(self):
        ...
        await self.bot.change_presence(
            status=discord.Status.online,
            activity=discord.Activity(type=discord.ActivityType.watching, name="Epic stuff is coming soon to JDBot."),
        )
        await asyncio.sleep(40)
    ...
    async def about(self, ctx):
        await ctx.send(
            "this bot is meant to test out new features of discord.py(from beta) and also new features in general and I want to be able to test them in discord.py, so I can get cricistim amd such."
        )
